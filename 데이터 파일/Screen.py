...
class Screen:
    ...
    def message_content(self, num):
        ...
        print(data[num][1],num)
        if data[num][1]=="재계약 추천" and num==0 and self.contract_data!=0:
            contract_label1 = Get_label.image_button(
                self.Gui, "contract.png", 1022, 150 + (50 * 0), lambda: do_contract(self.contract_data[0][0],self.contract_data[0][1],self.contract_data[0][2],"Players")
            )
            contract_label2 = Get_label.image_button(
                self.Gui, "contract.png", 1022, 150 + (50 * 1), lambda: do_contract(self.contract_data[1][0],self.contract_data[1][1],self.contract_data[1][2],"Coaches")
            )
            contract_label3 = Get_label.image_button(
                self.Gui, "contract.png", 1022, 150 + (50 * 2), lambda: do_contract(self.contract_data[2][0],self.contract_data[2][1],self.contract_data[2][2],"Staffs")
            )
        if data[num][1]=="비서":
            jirun_label1 = Get_label.image_button(
                self.Gui, "jirun.jpg", 500, 340, self.no_action
            )
    ...
    def financial(self):
        ...
        if fan_think[1]==0:
            self.con3 = Get_label.image_label_text(
                self.Gui, "fi5-2.png", 922, 288, f"{fan_think[0]}", "#472f91", ("고도 M", 12)
            )
        else:
            self.con3 = Get_label.image_label_text(
                self.Gui, "fi5-1.png", 922, 288, f"{fan_think[0]}", "#472f91", ("고도 M", 12)
            )
