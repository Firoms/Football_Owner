...
def msg_contact():
    ...
    db = sqlite3.connect(f"DB/FO_savefile3.db")
    cursor = db.cursor()
    cursor.execute(f'SELECT * FROM Players WHERE Team=="{team}" AND Contract=="1" ORDER BY random()')
    ...
    cursor.execute(f'SELECT * FROM Coaches WHERE Team=="{team}" AND Contract=="1" ORDER BY random()')
    ...
    cursor.execute(f'SELECT * FROM Staffs WHERE Team=="{team}" AND Contract=="1" ORDER BY random()')
...
def do_contract(seq,con,mon,type):
    ...
    if Warning_message == True:
        db = sqlite3.connect(f"DB/FO_savefile3.db")
        cursor = db.cursor()
        cursor.execute(f'UPDATE {type} SET Contract="{con}", Money="{mon}" WHERE Seq=="{seq}"')
        db.commit()
