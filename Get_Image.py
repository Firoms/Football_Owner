...
class Get_image:
    ...
    def image_label_text(Gui, file_name, x, y, text):
        img_path = os.path.join(os.getcwd(), "images")
        final_path = os.path.join(img_path, file_name)
        image = ImageTk.PhotoImage(file=final_path)
        image_label = Label(Gui.Gui, text=text)
        image_label.configure(image=image)
        image_label.image = image
    ...
    def image_button_text(Gui, file_name, x, y, command, text):
        img_path = os.path.join(os.getcwd(), "images")
        final_path = os.path.join(img_path, file_name)
        image = ImageTk.PhotoImage(file=final_path)
        image_button = Button(
            Gui.Gui, overrelief=SOLID,  command=command, text=text)
        image_button.configure(image=image)
        image_button.image = image
